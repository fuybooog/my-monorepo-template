...
import {
  ...
  SystemUserListResp,
} from '@/modules/user/system-user.dto'
...
export class SystemUserController {
  ...
  async pageOptionUser(
    @Query() query: SystemUserPageOptionDto,
  ): Promise<PaginatedResult<Partial<SystemUserPageResp>>> {
  ...
  async findUserById(@Param('id', ParseIntPipe) id: number): Promise<SystemUserResp | null> {
  ...
  async updateUserStatus(
    @Param('id') id: number,
    @Body() body: Pick<SystemUserUpdateDto, 'status'>,
  ) {
